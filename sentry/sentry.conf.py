...
# On Railway there is no Docker bridge network on eth0, so get_internal_network()
# returns () — guard against it so INTERNAL_SYSTEM_IPS doesn't contain an empty
# tuple that ipaddress.ip_network() then chokes on.
_internal_network = get_internal_network()
# On Railway eth0 has no IPv4 Docker subnet, so get_internal_network() returns ().
# Trust Railway's private network (IPv6 ULA fd00::/8) so the internal Relay's
# requests to /api/0/relays/* are recognised as internal. Locally, keep the
# detected subnet.
INTERNAL_SYSTEM_IPS = (_internal_network,) if _internal_network else ("fd00::/8",)

# Allow the bundled internal Relay to register/authenticate against this fresh
# self-hosted instance (baked credentials + Railway private network).
...
# Lean Railway deploy: store event bodies in Postgres (Django nodestore) instead
# of S3/SeaweedFS, so no object-storage service is needed. Fine for low volume;
# for high throughput, deploy an S3-compatible store (or SeaweedFS) and switch back
# to sentry_nodestore_s3.S3PassthroughDjangoNodeStorage.
SENTRY_NODESTORE = "sentry.nodestore.django.backend.DjangoNodeStorage"
...
SENTRY_OPTIONS["redis.clusters"] = {
    "default": {
        "hosts": {0: {"host": "redis.railway.internal", "password": "", "port": "6379", "db": "0"}}
    }
}
...
CACHES = {
    "default": {
        ...
        "LOCATION": ["memcached.railway.internal:11211"],
        "TIMEOUT": 3600,
...
DEFAULT_KAFKA_OPTIONS = {
    "bootstrap.servers": "kafka.railway.internal:9092",
